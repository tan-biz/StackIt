...
export default function Dashboard({ profile }: DashboardProps) {
  ...
  useEffect(() => { loadGames() }, [profile.id])
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        {actions.map((a) => (
          ...
          >
            <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${a.accent} flex items-center justify-center text-2xl mb-5 group-hover:scale-110 transition-transform`}>
              {a.icon}
            </div>
            ...
            {games.map((game) => (
              ...
              >
                ...
                <div className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase ${
                  ...
                  game.status === 'completed' ? 'bg-gray-600/30 text-gray-400' :
      ...
      {showCreate && (
        <CreateGameModal
          ...
          onCreated={() => { setShowCreate(false); loadGames() }}
        />
      )}
      {showJoin && (
        <JoinGameModal
          ...
          onJoined={() => { setShowJoin(false); loadGames() }}
        />
      )}
    </div>
  )
